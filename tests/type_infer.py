...
def pl_type_infer(a, b):
    ...
    out = np.empty((10,20,40,), int)
    out = plmap(lambda x, y: x + y, list_a, list_b[:,:,:,2])

    out = np.empty((5,3,6,), int)
    out = plmap(lambda x, y: x + y, list_a[:5][ :3, :6], \
                                    list_b[::2, ::7, 3:9, 2])
    # out = plmap(lambda x, y: x + y, a, c[0:5][1:3:-2])
