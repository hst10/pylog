...
if __name__=="__main__":
    ...
    num_iter = 10
    num_cluster = 5
    points=np.load(os.path.join("tests","golden_reference","kmeans_points_init.npy"))
    means=np.load( os.path.join("tests","golden_reference","kmeans_means_init.npy"))
    #labels = np.zeros(num_point,dtype=np.int32)#TODO: use np.zeros instead
    labels = np.load(os.path.join("tests","golden_reference","kmeans_labels_init.npy"))
    ...
    points_2=np.load(os.path.join("tests","golden_reference","kmeans_points.npy"))
    means_2=np.load( os.path.join("tests","golden_reference","kmeans_means.npy"))
    labels_2=np.load(os.path.join("tests","golden_reference","kmeans_labels.npy"))

    
    print(np.equal(points,points_2).all())
    print(np.equal(means,means_2).all())
    print(np.equal(labels,labels_2).all())